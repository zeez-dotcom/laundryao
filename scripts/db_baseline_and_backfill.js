...
async function main() {
  ...
  if (missing.includes('branch_delivery_areas')) {
    await client.query('CREATE TABLE IF NOT EXISTS "branch_delivery_areas" ("branch_id" varchar(255) NOT NULL, "area_id" varchar(255) NOT NULL, CONSTRAINT "branch_delivery_areas_branch_id_area_id_pk" PRIMARY KEY("branch_id","area_id"))');
  }

  if (missing.includes('delivery_account_branches')) {
    await client.query('CREATE TABLE IF NOT EXISTS "delivery_account_branches" ("delivery_account_id" varchar(255) NOT NULL, "branch_id" varchar(255) NOT NULL, CONSTRAINT "delivery_account_branches_delivery_account_id_branch_id_pk" PRIMARY KEY("delivery_account_id","branch_id"))');
  }

  if (missing.includes('driver_locations')) {
    await client.query('CREATE TABLE IF NOT EXISTS "driver_locations" ("driver_id" varchar(255) NOT NULL, "lat" numeric(9, 6) NOT NULL, "lng" numeric(9, 6) NOT NULL, "timestamp" timestamp DEFAULT CURRENT_TIMESTAMP NOT NULL, CONSTRAINT "driver_locations_driver_id_timestamp_pk" PRIMARY KEY("driver_id","timestamp"))');
  }
